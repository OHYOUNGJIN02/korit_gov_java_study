...
public class StreamApi {
    public static void main(String[] args) {
        ...
        List<Integer> nunbers =
                Arrays.asList(1,2,3,4,5);

        //Map () => 내부 요소들에게 각각 메소드를 적용하는 형태
        List<Integer> multiNumbers = nunbers.stream().map(number -> number * 2).toList();
        System.out.println("multiNumbers:"+ multiNumbers);

        List<String> englishNames = Arrays.asList("james", "alice", "smith", "pascal", "harry", "anya");
        //a로 시작하는 애들로 거르고 거르고 다 대문자로 변경
        List<String> result = englishNames.stream().filter(name -> name.startsWith("a"))
